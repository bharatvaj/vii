 ...
 *
 * $Header: /usr/build/vile/vile/filters/RCS/ecrypt.c,v 1.11 2010/07/13 15:00:57 tom Exp $
 *
 */
...
/**  our autokey (a special case of the running key) is being
	...
	text, and salt.   **/
	    ...
	    key = key + key + (ULONG) cc + (ULONG) (*bptr) + (ULONG) salt;
	}
...
filecrypt(FILE *ifp, char *key, int mailmode)
{
	...
	if (!mailmode)
	    ue_crypt(buf, (unsigned) strlen(buf));
